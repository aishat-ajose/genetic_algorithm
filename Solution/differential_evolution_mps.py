...
from differential_evolution_mps_function import ModifiedPollSearch

from differential_evolution_functions import *

class DifferentialEvolutionMPS:
    ...
    def differentialEvolution(self):
        ...
        while(self.iter <= self.max_iteration ):
            for target_index in range(self.population_size):
                ...
                searched_vector =  mps.modifiedPollSearch()
                
                selector_vector = selection(
                    target_index= target_index,
                    trial_vector= searched_vector,
                    fitness_function= self.fitness_function,
                    population= self.population,
                    target_vector= target_vector
                )
...
# a = DifferentialEvolutionMPS(population_size=500, crossover_probability=0.9, scale_factor= 0.8, max_iter=200, benchmark_function=Rastrigin())
